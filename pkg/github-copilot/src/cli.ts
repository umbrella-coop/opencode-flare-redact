...
import { guardFor, preToolUse, postToolUse, userPromptTransformed } from './hook.js';
...
async function main(): Promise<void> {
  ...
  try {
    ...
    process.exit(0); // malformed input → fail open, never crash the agent
  }
  ...
  switch (mode) {
    ...
    case 'userPromptTransformed':
      out = userPromptTransformed(input, guard);
      break;
    case 'verify': {
